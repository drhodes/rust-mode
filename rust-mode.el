        ...
        (case rust-tcat
          ...
          (?\{
           ...
           (let ((newcx (rust-push-context st ?\} (current-column))))
             (if (eq cx-info 'alt-2)
                 (setf (rust-context-info newcx) 'alt-outer)
               (dolist (cx (cdr (rust-state-context st)))
                 (when (eq (rust-context-type cx) ?\})
                   (when (eq (rust-context-info cx) 'alt-outer)
                     (setf (rust-context-info newcx) 'alt-inner))
                   (return))))))
          ((?\[ open-attr)
           (let ((newcx (rust-push-context st ?\] (current-column))))
